...
fun respondToMsg am client = Socket.output client (evalJson am (Socket.inputAll client))

fun handleIncoming am listener =
    let val client = Socket.accept listener
     in respondToMsg am client;
        Socket.close client
    end
...
fun startServer ini =
    let val port = lookupOrDefault ini Int.fromString Int.toString "port" 5000
     ...
     in case iniServerAm ini of 
        ...
        | Ok am => loop (handleIncoming am) (Socket.listen port qLen)
    end
