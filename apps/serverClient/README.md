...
Finally, run the client. If you are running it on the same device, you can run `./client localhost`. If the server is on another device, pass the ip address of that device instead. The client assumes the server is listening on port 5000.

Use ctrl+c (SIGINT) to stop the server and `testProc`.
