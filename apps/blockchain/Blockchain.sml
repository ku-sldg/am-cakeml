...
(*** [ABI Encoding](https://docs.soliditylang.org/en/develop/abi-spec.html) ***)
    ...
    *)
    fun encodeAddress str =
        (* we're convert to and from a `BString.bstring` in order to check that
        * the argument is a hexadecimal string. We'd rather through an error
...
(******************* Communicating with the Smart Contract *******************)
    ...
    in
        ...
        *
        * Queries Ethereum client located at host `host` and port number `port`,
        * calling the `addAuthorizedUser` method of our smart contract, located
        ...
        *
        * Queries Ethereum client located at host `host` and port number `port`,
        * calling the `removeAuthorizedUser` method of our smart contract,
        ...
        * response to this particular request. All three `recipient`, `sender`,
        * `address` need to be hexadecimal strings prefixed by `0x` and
        * representing 20 bytes.
        *)
