...
DEMO_FILES=../tests/DemoFiles/ExampleInlineAuth
...
SERVER_P3_FORM_MAN=$DEMO_FILES/FormalManifest_P3.json    #$SERVER_P0_FORM_MAN
SERVER_AM_LIB=$DEMO_FILES/Test_Am_Lib_ExampleInlineAuth.sml
...
CLIENT_TERM_FILE_JSON=$DEMO_FILES/ClientCvmTermExampleInlineAuth.json

MANGEN_TERMS_FILE=$DEMO_FILES/ServerPlcTermsExampleInlineAuth.json
MANGEN_EVS_FILE=$DEMO_FILES/ServerPlcEvsExampleInlineAuth.json
...
if [[ "$PWD" == */am-cakeml/tests ]]; then
  ...
  # Start the P3 server
  tmux send-keys -t 2 "( $BUILT_SERVER_AM_P3 -m $SERVER_P3_FORM_MAN -k $SERVER_PRIV_KEY )" Enter
