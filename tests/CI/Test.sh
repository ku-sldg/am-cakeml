...
usage() {
  echo "Usage: $0 -t [micro] (-h (headless)) -m <path-to-model_args.json> [-a <path-to-asps>]"
  exit 1
}
...
if [[ "$REPO_ROOT" == */am-cakeml ]]; then
  ...
  if [[ $HEADLESS -eq 0 ]]; then
    ...
    $CLIENT_AM_EXEC -t $TERM_FILE -s $TEST_ATT_SESS -a $MODEL_ARGS_FILE $PROVISION_ARG  > $GENERATED/output_resp.json
