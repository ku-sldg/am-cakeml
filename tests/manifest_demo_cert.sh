...
CLIENT_CONC_MAN=$SERVER_P0_FORM_MAN #$DEMO_FILES/concrete_Manifest_P0.json
...
if [[ "$PWD" == */am-cakeml/tests ]]; then
  ...
  tmux send-keys -t 3 \
    "($MAN_COMP -c $CLIENT_TERM_FILE -o $CLIENT_EXE_NAME -om $CLIENT_CONC_MAN -m $CLIENT_FORM_MAN -l $CLIENT_AM_LIB) && \
