...
if [[ "$PWD" == */am-cakeml/tests ]]; then
  ...
  # Start the P0 client
  tmux send-keys -t 3 "sleep 1 && ($BUILT_CLIENT_AM_P0 -m $CLIENT_P0_FORM_MAN -k $CLIENT_PRIV_KEY -cs)" Enter

  # Start the P1 client
  tmux send-keys -t 4 "sleep 1 && ($BUILT_CLIENT_AM_P1 -m $CLIENT_P1_FORM_MAN -k $CLIENT_PRIV_KEY -cs)" Enter
