...
if [[ "$PWD" == */am-cakeml/tests ]]; then
  ...
  # Start the P3 server
  tmux send-keys -t 3 "($BUILT_SERVER_AM_P3 -m $SERVER_P3_FORM_MAN -k $SERVER_PRIV_KEY )" Enter

  # Start the P4 server
  tmux send-keys -t 4 "($BUILT_SERVER_AM_P4 -m $SERVER_P4_FORM_MAN -k $SERVER_PRIV_KEY )" Enter

  # Start the P0 client
  tmux send-keys -t 5 "($BUILT_CLIENT_AM_P0 -m $CLIENT_P0_FORM_MAN -k $CLIENT_PRIV_KEY -cs)" Enter
