...
val aspMapping = (mapC_from_pairList 
  [
    ...
    (attest1_id,
      fn par => fn plc => fn bs => fn rawEv => attest1_asp_stub par rawEv),
    (attest2_id,
      fn par => fn plc => fn bs => fn rawEv => attest2_asp_stub par rawEv),
    (appraise_id,
    ...
    (check_ssl_sig_aspid,
      fn par => fn plc => fn bs => fn rawEv => check_ssl_sig_asp_stub par rawEv)
  ]
...
val appAspMapping = (mapC_from_pairList [
      ...
      ((Coq_pair coq_P2 appraise_inline_id), 
        fn par => fn p => fn bs => fn rawEv => appraise_appraise_meas_asp_stub par p bs rawEv),

      ((Coq_pair coq_P1 appraise_inline_id), 
        fn par => fn p => fn bs => fn rawEv => appraise_appraise_meas_asp_stub par p bs rawEv),
      ...
      ((Coq_pair coq_P1 check_ssl_sig_aspid), 
        fn par => fn p => fn bs => fn rawEv => appraise_check_ssl_sig_asp_stub par p bs rawEv)
