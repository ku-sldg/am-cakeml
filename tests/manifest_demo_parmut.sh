...
CLIENT_P0_TERM_FILE_JSON=$DEMO_FILES/ClientCvmTermParmutP0.json
CLIENT_P1_TERM_FILE_JSON=$DEMO_FILES/ClientCvmTermParmutP1.json

MANGEN_TERMS_FILE=$DEMO_FILES/ServerPlcTermsParmut.json
MANGEN_EVS_FILE=$DEMO_FILES/ServerPlcEvsParmut.json
...
if [[ "$PWD" == */am-cakeml/tests ]]; then
  ...
  # Start the P3 server (appraisal server for P0 and P1 clients)
  ...
  # Start the P0 server
  tmux send-keys -t 1 "($BUILT_SERVER_AM_P0 -m $SERVER_P0_FORM_MAN -k $SERVER_PRIV_KEY )" Enter

  # Start the P1 server
  tmux send-keys -t 2 "($BUILT_SERVER_AM_P1 -m $SERVER_P1_FORM_MAN -k $SERVER_PRIV_KEY )" Enter
