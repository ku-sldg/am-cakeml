...
/**
 * This function is a wrapper around the popen function in C. 
 * It takes a command as a string and executes it in a shell. 
 * The output of the command is then piped into a buffer and copied into the output buffer.
...
void ffipopen(const uint8_t * c, const long clen, uint8_t * a, const long alen) {
    ...
    strncpy((char*)a, output, MAX_BUFFER_SIZE);
    free(output);
}
