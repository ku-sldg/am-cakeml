...
fun socketDispatch_json (fromPl : coq_Plc) (jsonPlcMap : jsonPlcMap) (toPl : coq_Plc) (et : coq_Evidence) (ev : (bs list)) (t : coq_Term) =
    let val plc_specific_mapping = case (Map.lookup jsonPlcMap (plToString toPl)) of
        ...
        val req  = (REQ_json fromPl toPl jsonPlcMap t et ev)
        ...
        val (RES _ _ resev) = (serverSend_json fd req; serverRcv fd)
     in Socket.close fd;
        resev
    end
...
fun am_sendReq_json t fromPl toPl et evv =
    let (* val fromPl = O *)
        val myJson = get_json ()
        val jsonMap = json_config_to_map myJson
        val jsonPMap = extractJsonPlcMap jsonMap
        val resev = socketDispatch_json fromPl jsonPMap toPl et evv t
    in
        (print ("Sent term:\n" ^ termToString t ^
